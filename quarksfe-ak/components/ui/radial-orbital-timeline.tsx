...
export default function RadialOrbitalTimeline({
  ...
  }, [autoRotate, currentFeatureIndex, isClient]);
  ...
  const calculateNodePosition = (index: number, total: number) => {
    // Use static angle for SSR, dynamic for CSR
    const angle = isClient ? ((index / total) * 360 + rotationAngle) % 360 : ((index / total) * 360) % 360;
    const radius = 210;
